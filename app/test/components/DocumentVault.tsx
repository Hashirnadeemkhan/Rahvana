...
export function DocumentVault({ 
    ...
    const handleFileChange = (doc: string, e: React.ChangeEvent<HTMLInputElement>) => {
    ...
    return (
        <div className="fixed inset-0 z-[1000] flex items-center justify-center p-4">
            ...
            {/* Content */}
            <div className="relative bg-white rounded-2xl w-full max-w-[800px] max-h-[90vh] overflow-hidden shadow-2xl flex flex-col animate-in fade-in zoom-in duration-200">
                {/* Header */}
                <div className="p-6 border-b border-slate-200 flex justify-between items-center bg-white shrink-0">
                    <div>
                        ...
                        <p className="text-slate-500 text-sm mt-1">Track and manage your required documents.</p>
                    </div>
                    <button 
                        onClick={onClose}
                ...
                <div className="p-6 overflow-y-auto flex-grow bg-slate-50/30">
                    <div className="bg-[#ebf5f4] text-[#0f766e] p-4 rounded-xl text-sm border border-[#0d9488]/20 mb-6 flex gap-3">
                        ...
                        <div>
                            <strong>Privacy Note:</strong> For this demo, files are processed locally. In production, documents are encrypted and stored in your secure Document Vault.
                        </div>
                    </div>
                    ...
                    <div className="space-y-4">
                        {roadmapData.documents.map((doc) => {
                            ...
                            return (
                                <div key={doc} className={`p-5 rounded-2xl border transition-all duration-200 bg-white ${
                                    isChecked ? 'border-[#0d9488] shadow-sm' : 'border-slate-100'
                                }`}>
                                    <div className="flex flex-col md:flex-row md:items-center justify-between gap-4 mb-4">
                                        <div className="flex items-center gap-4">
                                            ...
                                            <div>
                                                <h4 className={`font-bold text-base transition-colors ${isChecked ? 'text-slate-900' : 'text-slate-500'}`}>
                                                    {doc}
                                                </h4>
                                                <div className="flex gap-2 mt-1">
                                                    <span className={`text-[11px] font-bold px-2 py-0.5 rounded-full uppercase tracking-wider ${
                                                        isChecked ? 'bg-[#ebf5f4] text-[#0d9488]' : 'bg-slate-100 text-slate-400'
                                                    }`}>
                                    ...
                                    <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                                        <div className="space-y-2">
                                            <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block px-1">Notes</label>
                                            <input 
                                        ...
                                        <div className="space-y-2">
                                            <label className="text-[11px] font-bold uppercase tracking-wider text-slate-400 block px-1">File Attachment</label>
                ...
                {/* Footer */}
                <div className="p-6 border-t border-slate-200 bg-slate-50 shrink-0 flex justify-end">
                    <button 
                        onClick={onClose}
                        className="px-8 py-3 bg-[#334155] text-white rounded-lg font-bold hover:bg-[#1e293b] transition-all shadow-md hover:-translate-y-px"
                    >
                        Close Vault
                    </button>
                </div>

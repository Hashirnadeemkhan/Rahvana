...
const DocumentNeedStep = ({ selected, onSelect }: DocumentNeedStepProps) => {
  const { title, description, types } = (guideData.wizard as any).document_need;
    ...
    <motion.div initial={{ opacity: 0, y: 12 }} animate={{ opacity: 1, y: 0 }}>
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 gap-5 mb-8">
        {types.map((option: any) => {
          const Icon = ICONS[option.id] || FilePlus;
          ...
          return (
            <motion.button
              ...
              className={`
                p-6 rounded-2xl text-left border-2 transition-all flex flex-col gap-4 font-['Plus_Jakarta_Sans',system-ui]
                ${isSelected 

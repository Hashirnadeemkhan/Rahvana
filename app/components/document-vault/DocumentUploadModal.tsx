  ...
  return (
    <Dialog open={open} onOpenChange={handleClose}>
      <DialogContent className="max-w-2xl">
        <DialogHeader>
          <DialogTitle>Upload Document: {documentDef.name}</DialogTitle>
        </DialogHeader>
        ...
        <div className="space-y-4">
          ...
          <div>
            ...
            <div
              ...
              className="border-2 border-dashed rounded-lg p-8 text-center cursor-pointer hover:border-brand transition-colors"
            >
              {selectedFile ? (
          ...
          {selectedFile && config && (
            <div className="bg-blue-50 dark:bg-blue-950 border border-blue-200 dark:border-blue-800 rounded-lg p-4 space-y-3">
              ...
              {/* Display name */}
              <div>
                <p className="text-xs text-blue-700 dark:text-blue-300 mb-1">Display Name:</p>
              ...
              <div>
                <p className="text-xs text-blue-700 dark:text-blue-300 mb-1">Standardized Filename:</p>
                <div className="bg-white dark:bg-blue-900/50 p-2 rounded">
                  <code className="text-xs text-blue-800 dark:text-blue-200 break-all">
                    {`${(config.caseId || 'CASE').replace(/[^a-zA-Z0-9]/g, '').toUpperCase()}_${role}_${
                      (role === 'PETITIONER'
                       ...
                       .toUpperCase()
                       .substring(0, 30)
                    }_${documentDef.key}_${new Date().toISOString().split('T')[0]}_v1.${
              ...
              {(!config.caseId || !config.petitionerName || !config.beneficiaryName) && (
                <div className="bg-red-50 dark:bg-red-900/50 border border-red-200 dark:border-red-800 rounded p-2">
                  <p className="text-xs text-red-800 dark:text-red-200">
                    ⚠️ <strong>Warning:</strong> Set Case Information in left panel for proper USCIS naming!
                  </p>
                </div>
              )}
            </div>

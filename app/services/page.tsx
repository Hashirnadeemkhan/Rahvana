...
const ConsultationBookingPage = () => {
  ...
  const issueCategories = [
    ...
    { value: 'priority-date', label: 'Priority Date Questions', group: 'Petition & Application Filing' },
    { value: 'birth-certificate', label: 'Birth Certificate Issues (NADRA)', group: 'Document Preparation & Evidence' },
    { value: 'marriage-certificate', label: 'Marriage Certificate Authentication', group: 'Document Preparation & Evidence' },
    { value: 'police-clearance', label: 'Police Clearance Certificates', group: 'Document Preparation & Evidence' },
    { value: 'translation', label: 'Document Translation & Certification', group: 'Document Preparation & Evidence' },
    { value: 'bona-fide-evidence', label: 'Evidence of Bona Fide Relationship', group: 'Document Preparation & Evidence' },
    { value: 'financial-docs', label: 'Financial Documentation', group: 'Document Preparation & Evidence' },
    { value: 'nvc-welcome', label: 'NVC Welcome Letter/Case Creation', group: 'NVC Processing' },
    { value: 'ds260', label: 'DS-260 Completion & Submission', group: 'NVC Processing' },
    { value: 'i864', label: 'I-864 Affidavit of Support', group: 'NVC Processing' },
    { value: 'nvc-fees', label: 'NVC Fee Payment Issues', group: 'NVC Processing' },
    { value: 'dq-status', label: 'Documentarily Qualified Status', group: 'NVC Processing' },
    { value: 'interview-scheduling', label: 'Interview Appointment Scheduling', group: 'Embassy Interview & Processing' },
    { value: 'interview-prep', label: 'Interview Preparation', group: 'Embassy Interview & Processing' },
    { value: 'medical-exam', label: 'Medical Examination', group: 'Embassy Interview & Processing' },
    { value: 'interview-outcome', label: 'Interview Outcome Explanation', group: 'Embassy Interview & Processing' },
    { value: '221g-white', label: '221(g) White Slip (Documents Missing)', group: 'Administrative Processing & 221(g)' },
    { value: '221g-blue', label: '221(g) Blue/Pink Slip (Background Check)', group: 'Administrative Processing & 221(g)' },
    { value: 'ap-timeline', label: 'Administrative Processing Timeline', group: 'Administrative Processing & 221(g)' },
    { value: 'ceac-status', label: 'CEAC Status Interpretation', group: 'Administrative Processing & 221(g)' },
    { value: 'income-requirements', label: 'I-864 Income Requirements', group: 'Financial Sponsorship' },
    { value: 'joint-sponsor', label: 'Joint Sponsor Requirements', group: 'Financial Sponsorship' },
    { value: 'public-charge', label: 'Public Charge Considerations', group: 'Financial Sponsorship' },
    { value: 'visa-bulletin', label: 'Visa Bulletin Interpretation', group: 'Visa Bulletin & Wait Times' },
    { value: 'retrogression', label: 'Category Retrogression', group: 'Visa Bulletin & Wait Times' },
    { value: 'aging-out', label: 'Aging Out & CSPA Protection', group: 'Visa Bulletin & Wait Times' },
  ];

  const visaCategories = [
    { value: 'ir1-cr1', label: 'IR1/CR1 - Spouse of U.S. Citizen', group: 'Immediate Relatives' },
    { value: 'ir2-cr2', label: 'IR2/CR2 - Child of U.S. Citizen', group: 'Immediate Relatives' },
    { value: 'ir5', label: 'IR5 - Parent of U.S. Citizen', group: 'Immediate Relatives' },
    { value: 'f1', label: 'F1 - Unmarried Adult Child of USC', group: 'Family Preference' },
    { value: 'f2a', label: 'F2A - Spouse/Child of LPR', group: 'Family Preference' },
    { value: 'f2b', label: 'F2B - Unmarried Adult Child of LPR', group: 'Family Preference' },
    { value: 'f3', label: 'F3 - Married Child of USC', group: 'Family Preference' },
    { value: 'f4', label: 'F4 - Sibling of USC', group: 'Family Preference' },
    { value: 'k1', label: 'K-1 - Fiance(e) of USC', group: 'Fiance/Spousal' },
    { value: 'k2', label: 'K-2 - Child of K-1 Holder', group: 'Fiance/Spousal' },
    { value: 'dv', label: 'DV - Diversity Visa', group: 'Other' },
    { value: 'other', label: 'Other / Not Sure', group: 'Other' },
  ];

  const caseStages = [
    { value: 'pre-filing', label: 'Pre-Filing / Planning', group: 'USCIS Phase' },
    { value: 'petition-filed', label: 'Petition Filed', group: 'USCIS Phase' },
    { value: 'rfe-received', label: 'RFE Received', group: 'USCIS Phase' },
    { value: 'petition-approved', label: 'Petition Approved', group: 'USCIS Phase' },
    ...
    { value: 'interview-scheduled', label: 'Interview Scheduled', group: 'NVC Phase' },
    { value: 'pre-interview', label: 'Preparing for Interview', group: 'Embassy Phase' },
    { value: 'interview-completed', label: 'Interview Completed', group: 'Embassy Phase' },
    { value: '221g-ap', label: '221(g) / Admin Processing', group: 'Embassy Phase' },
  ...
  const fetchUserBookings = async (query = '') => {
    setLoading(true);
    try {
      const url = query ? `/api/consultation?search=${encodeURIComponent(query)}` : '/api/consultation';
            ...
            <div className="grid grid-cols-1 md:grid-cols-2 gap-6">
              <div>
                ...
                >
                  ...
                  {[...new Set(issueCategories.map(c => c.group))].map(group => (
                    <optgroup key={group} label={group!}>
                      {issueCategories.filter(c => c.group === group).map(c => (
              ...
              <div>
                ...
                >
                  ...
                  {[...new Set(visaCategories.map(c => c.group))].map(group => (
                    <optgroup key={group} label={group!}>
                      {visaCategories.filter(c => c.group === group).map(c => (
                        <option key={c.value} value={c.value}>{c.label}</option>
                      ))}
                    </optgroup>
                  ))}
            ...
            <div className="grid grid-cols-1 md:grid-cols-3 gap-6">
              <div>
                ...
                >
                  ...
                  {[...new Set(caseStages.map(s => s.group))].map(group => (
                    <optgroup key={group} label={group!}>
                      {caseStages.filter(s => s.group === group).map(s => (
              ...
              <div>
                ...
                <select
                  ...
                  onChange={(e) => handleInputChange('embassy_consulate', e.target.value)}
                  className="w-full px-4 py-3 border border-gray-300 rounded-lg focus:ring-2 focus:ring-teal-500 transition"
                  required
                >
          ...
          <div className="space-y-6">
            ...
            <div>
              ...
              <label className="border-2 border-dashed border-gray-200 rounded-2xl p-8 hover:border-teal-400 hover:bg-teal-50/30 transition-all group cursor-pointer text-center block">
                ...
                <p className="text-sm font-bold text-gray-700">Click to add documents</p>
                <p className="text-xs text-gray-500 mt-1">Notices, letters, or forms relevant to your case</p>
              </label>
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
              {availableSlots.length > 0 ? (
                ...
                <div className="col-span-1 sm:col-span-2 lg:col-span-3 text-center py-12 border-2 border-dashed border-gray-200 rounded-2xl">
                  ...
                  <p className="text-gray-500">No consultation slots are currently available.</p>
                  <p className="text-xs text-gray-400 mt-1">Please check back later or contact us for assistance.</p>
                </div>
              )}
            </div>
            {formData.selected_slot && (
              <div className="space-y-4">
                <p className="text-sm font-medium">Alternative Slots (Optional - Max 2)</p>
                <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-3">
                  {availableSlots.filter(s => s.available && s.date.getTime() !== formData.selected_slot?.getTime()).slice(0, 6).map(slot => (
                    <div
                      key={`alt-${slot.id}`}
                      onClick={() => handleAlternateSlotSelect(slot)}
                      className={cn(
                        "border rounded-lg p-3 cursor-pointer transition text-xs",
                        formData.alternate_slots.some(s => s.getTime() === slot.date.getTime()) ? "border-blue-500 bg-blue-50" : "border-gray-200 hover:bg-gray-50"
                      )}
                    >
        ...
        <div className="bg-gradient-to-r from-teal-800 to-teal-600 text-white py-12 mb-10 shadow-lg relative overflow-hidden">
          <div className="max-w-4xl mx-auto px-4 relative z-10">
            <div className="flex justify-between items-center mb-4">
              <h1 className="text-3xl font-black tracking-tight">My <span className="text-teal-200">Requests</span></h1>
              <Button onClick={() => setActiveView('booking')} variant="outline" className="text-white border-white/30 bg-white/10 hover:bg-white/20">
                New Request
              </Button>
            </div>
            <p className="text-teal-50 font-light">Track and manage your existing consultation requests.</p>
          </div>
            ...
            <div className="p-6 border-b bg-gray-50 flex flex-col sm:flex-row gap-4 justify-between items-center">
              <div className="relative w-full sm:max-w-md">
                <input
                  ...
                  onKeyDown={(e) => e.key === 'Enter' && fetchUserBookings(searchQuery)}
              ...
              <Button onClick={() => fetchUserBookings(searchQuery)} className="bg-teal-600 text-sm py-2">
                Search
              </Button>
            </div>
                      ...
                      <div>
                        <div className="flex items-center gap-2 mb-1">
                          ...
                          <span className={cn(
                            ...
                            booking.status === 'alternatives_proposed' ? "bg-purple-100 text-purple-700" :
                            "bg-gray-100 text-gray-700"
                          )}>
                            {booking.status?.replace('_', ' ')}
                          </span>
                        </div>
                        <h4 className="font-bold text-gray-900">{booking.full_name}</h4>
                        <p className="text-sm text-gray-500">{booking.visa_category} • {booking.issue_category}</p>
                      </div>
                      <div className="text-right flex flex-col items-end gap-2">
                        <div>
                          <p className="text-xs text-gray-400 mb-1">Requested Date</p>
                          <p className="text-sm font-medium">
                            {new Date(booking.selected_slot).toLocaleString(undefined, { month: 'short', day: 'numeric', hour: 'numeric', minute: '2-digit' })}
                          </p>
                        </div>
                        
                        {booking.status === 'alternatives_proposed' && booking.alternate_slots && booking.alternate_slots.length > 0 && (
                          <div className="mt-4 p-4 bg-teal-50 rounded-xl border border-teal-100 text-left w-full sm:w-auto">
                            ...
                            <div className="flex flex-wrap gap-2">
                              {booking.alternate_slots.map((slot: string, idx: number) => (
                                ...
                                >
                                  {new Date(slot).toLocaleDateString(undefined, { month: 'short', day: 'numeric' })} @ {new Date(slot).toLocaleTimeString([], { hour: '2-digit', minute: '2-digit' })}
                                </Button>
                              ))}
                            </div>
                            {booking.admin_notes && (
      ...
      <div className="min-h-screen bg-gray-50 flex items-center justify-center p-4">
        <div className="max-w-md w-full bg-white rounded-2xl shadow-xl overflow-hidden text-center">
          ...
          <div className="p-8">
            ...
            <p className="text-gray-600 mb-6">Your reference ID is: <span className="font-mono font-bold text-teal-700">{referenceId}</span></p>
            <div className="space-y-3">
              <Button onClick={resetForm} className="w-full bg-teal-600">Book Another</Button>
  ...
  return (
    <div className="min-h-screen bg-gray-50 pb-12">
      <div className="bg-gradient-to-r from-teal-800 to-teal-600 text-white py-16 mb-10 relative overflow-hidden">
        <div className="max-w-4xl mx-auto px-4 relative z-10">
          <div className="flex flex-col md:flex-row md:items-center justify-between gap-6">
            <div>
              <h1 className="text-4xl md:text-5xl font-black mb-4">Book <span className="text-teal-200">Consultation</span></h1>
              <p className="text-xl text-teal-50 max-w-2xl font-light">Personalized guidance for your U.S. immigration journey.</p>
            </div>
              ...
              {[
                ...
                <div key={item.step} className="flex flex-col items-center gap-2">
                  ...
                  <span className={cn(
                    ...
                    currentStep >= item.step ? "text-teal-900" : "text-gray-400"
                  )}>
                    {item.label}
                  </span>
                </div>
            ...
            <div className="px-8 py-5 bg-gray-50 border-t flex justify-between items-center">
              ...
              {currentStep < 3 ? (
                <Button 
                  type="button" 
                  onClick={handleNext}
                  className="bg-teal-600 hover:bg-teal-700 gap-2 px-8"
                >

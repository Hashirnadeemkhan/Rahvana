...
# Load latest Visa Bulletin
with open("visa-checker-dates.json", "r") as f:
    BULLETIN = json.load(f)

# Historical movement for F3 Philippines (realistic)
HISTORICAL_F3_PH = [
    "2024-11", "2004-08-01",
    "2024-12", "2004-08-15",
    "2025-01", "2004-08-22",
...
def parse_visabulletin_date(date_str: str) -> str:
    """
    Converts Visa Bulletin format like '01DEC13' or '15JUL24' → '2013-12-01'
    """
    if date_str == "C" or date_str == "U":
        return date_str

    date_str = date_str.strip().upper()
    
    # Match: 01DEC13, 15JUL24, 22SEP04
...
def estimate_wait_f3_philippines() -> Dict:
    ...
    for i in range(1, len(HISTORICAL_F3_PH), 2):
        ...
        next_idx = i + 2
        curr_str = HISTORICAL_F3_PH[next_idx] if next_idx < len(HISTORICAL_F3_PH) else HISTORICAL_F3_PH[-1]
...
def check_status(input: dict):
    ...
    # Dynamic bulletin name
    bulletin_month = BULLETIN.get("month", "2025-11")
    bulletin_display = datetime.strptime(bulletin_month, "%Y-%m").strftime("%B %Y")
    ...
    if cutoff_raw == "C":
        ...
        # CORRECT LOGIC: Priority Date <= Cut-off Date → Current
        if user_dt <= cutoff_dt:
            ...
            if category == "F3" and country == "Philippines":
                ...
                if est["avg_movement"] > 0:
                    wait_years = round(days_behind / (est["avg_movement"] * 30), 1)
                    wait_estimate = {
